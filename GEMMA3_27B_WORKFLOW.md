# Complete Workflow: Gemma-3 27B Experiment Setup

**This workflow is identical to Gemma-3 12B, with only the model name changed to `gemma-3-27b-it`.**
...
# Pull latest changes (to get Gemma-3 27B support)
...
### Step 9: Setup HuggingFace Token

```bash
# Navigate to project root
cd /workspace/kernel-divergence-sad

# Check if token file already exists (from Gemma-3 12B experiment)
if [ -f token ]; then
    echo "Token file already exists - reusing it"
    cat token
else
...
**Important Notes**:
- If you ran Gemma-3 12B on the same instance, the `token` file should already exist
...
### Step 11: Verify GPU Availability
...
echo "=========================================="
echo "Gemma-3 27B Complete Setup"
echo "=========================================="
...
# Step 9: Check token file
echo "Step 9: Checking token file..."
cd /workspace/kernel-divergence-sad
if [ ! -f token ]; then
...
**Log file** (`run_gemma-3-27b-it.log`):
```
...
**Results file** (`out/stages_oversight_gemma-3-27b-it/results.tsv`):
...
8. ✅ Verify packages
9. ✅ Setup token file (reuse from 12B if available)
10. ✅ Verify model access
11. ✅ Verify GPU
12. ✅ Run experiment
13. ✅ Monitor progress
